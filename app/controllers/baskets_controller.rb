class BasketsController < ApplicationController
  ...
  def create
    @basket = Basket.new(name: params[:basket_name], user_id: current_user.id)
    if @basket.save
      render "/users/show"
    else
  ...
  def show
    ...
    @fundsbaskets = FundsBasket.where(basket_id: @basket.id)
    @funds = []
    @funds_in_fundsbasket = []
    @fund_names = []
    @fundsbaskets.each do |fundsbasket|
      @funds << fundsbasket.fund
    end
